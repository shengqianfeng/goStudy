...
func main(){
	...
	str := "hello北" 
	fmt.Println("str len=", len(str)) // 8个字节
	...
	fmt.Printf("bytes=%v\n", bytes)//输出的是，每一个字母的ASCII码

	//6)[]byte 转 字符串: str = string([]byte{97, 98, 99})
	str = string([]byte{97, 98, 99}) //ASCII码
	fmt.Printf("str=%v\n", str)//abc
	...
	str2 = "go go hello"
	str = strings.Replace(str2, "go", "北京", -1)//-1表示全部替换
	fmt.Printf("str=%v str2=%v\n", str, str2)//str2本身并没有发生变化
