...
func main() {
	...
	*/
	...
	fmt.Printf("----totalWeight type %T\n",totalWeight)//float64
	fmt.Printf("----6 type %T\n",6)//float64
	//fmt.Sprintf("%.2f", totalWeight / 6) 将 totalWeight / 6 四舍五入保留到小数点2返回值
	avgWeight := fmt.Sprintf("%.2f", totalWeight / 6)//注意--->这个6是具体常量，可以跟float64运算
	...
	//4.求出平均体重
	fmt.Printf("----totalWeight2 type %T\n",totalWeight2)//float64
	fmt.Printf("----len type %T\n",len(hens))//int
	avgWeight2 := fmt.Sprintf("%.2f", totalWeight2 / float64(len(hens)))  //注意--->len(hens)这个计算出来是int不可以跟float64运算
